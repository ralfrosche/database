...
public String[] ReadNRFromDB(String filter, boolean youth) {
 ...
 String constraint = "";
 if (filter != "") {
	 constraint = " WHERE Nachname like '%"+filter+"%' AND status <> 'ausgetreten'";
 }
 else {
 ...
 if (youth == true) {
	 ...
	 String sqlQuery = "SELECT Vorname, Nachname, mitgliedsnummer FROM mitglieder "+constraint+" ORDER BY Nachname" ;	   
...
String sqlQuery = "SELECT * FROM mitglieder where mitgliedsnummer = '"+ result_array[2] +"'"  ;	   
...
if (c.moveToFirst()){
    do{
    	...
    	member_array[8] = c.getString(c.getColumnIndex("mitgliedsnummer"));
...
if (c.moveToFirst()){
    do{
    	member_array[3] = c.getString(c.getColumnIndex("ehrenmitglieder"));
...
if (c.moveToFirst()){
    do{
    	member_array[4] = c.getString(c.getColumnIndex("foerdermitglied"));
...
if (c.moveToFirst()){
    do{
    	member_array[2] = c.getString(c.getColumnIndex("jugendliche"));
...
if (c.moveToFirst()){
    do{
    	member_array[6] = c.getString(c.getColumnIndex("austritte"));
	           ...
	           if (c.moveToFirst()){
	               do{
	                     temp_array.add(  c.getString(c.getColumnIndex("Lebensalter"))+
	                    		 " Jahre: " + c.getString(c.getColumnIndex("summe_alter")) 
